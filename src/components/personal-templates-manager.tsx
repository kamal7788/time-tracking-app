...
interface PersonalTemplatesManagerProps {
  clients: PersonalClient[]
}

export default function PersonalTemplatesManager({ clients }: PersonalTemplatesManagerProps) {
  ...
  const onAddClient = async (data: { name: string; description?: string }) => {
    ...
    try {
      ...
      setSuccess('Personal client created')
      resetClient()
  ...
  const onAddProject = async (data: { name: string; description?: string; clientId: string }) => {
    ...
    try {
      ...
      const result = await res.json()
      if (!res.ok) throw new Error(result.error)
      setSuccess('Personal project created')
      resetProject()
  ...
  const handleDeleteClient = async (id: string) => {
    if (!confirm('Delete this personal client?')) return
    try {
    ...
    <div className="space-y-4">
      ...
      {activeTab === 'clients' && (
        <div className="space-y-4">
          ...
          {isAddingClient && (
            <div className="card">
              <div className="card-header"><h3 className="font-semibold">New Personal Client</h3></div>
          ...
          <div className="space-y-4">
            {clients.length === 0 && !isAddingClient && (
              <div className="text-center py-8 text-gray-500">
                No personal clients yet. Create one to use as a template for time entries.
              </div>
            )}
            {clients.map((client) => (
      ...
      {activeTab === 'projects' && (
        <div className="space-y-4">
          ...
          {isAddingProject && (
            <div className="card">
              <div className="card-header"><h3 className="font-semibold">New Personal Project</h3></div>
              <div className="card-body">
          ...
          <div className="card">
            <div className="card-body">
              {clients.length === 0 ? (
                <p className="text-center text-gray-500 py-4">Create a personal client first.</p>
              ) : (
